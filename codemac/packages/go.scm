     ...
     `(#:modules ((guix build utils))
                 ...
                 (begin
                   ...
                   (let* ((tarballgz (assoc-ref %build-inputs "source"))
                          ...
                          (out-doc (string-append (assoc-ref %outputs "out") "/usr/share/doc"))
                     ...
                     (begin
                       ...
                       ;; (string-append "GO_LDFLAGS=-extldflags '-Wl,-rpath," gcclib " -Wl,-rpath," glibclib "'")
                       (zero? (system* "sed" "-i" (string-append "13i" patchelf " --set-interpreter" (string-append glibclib "/ld-linux-x86-64.so.2") "../bin/go") "./all.bash"))
                       ...
                       (zero? (system* "env"
                                       ...
                                       "bash" "./all.bash"))
                       (chdir "../..")
                       (for-each mkdir-p (list out-usrgo out-bin out-doc))
                       (for-each (lambda (x) (copy-recursively (string-append "go/" x) out-usrgo))
                                 (filter (lambda (x) (not (or (equal? x "bin") (equal? x "doc"))))
                                         (scandir "go")))
                       (copy-recursively "go/bin" out-bin)
                       (copy-recursively "go/doc" out-doc)
                       (symlink out-bin (string-append out-usrgo "/bin"))
                       (symlink out-doc (string-append out-usrgo "/doc")))))
